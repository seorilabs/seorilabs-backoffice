...
import { agentCompletionSchema } from "@/lib/control-plane/contracts";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = agentCompletionSchema.parse(await request.json());
    const result = await settleAgentRun({ ...body, workerId: principal.id, outcome: "complete", idempotencyKey });
