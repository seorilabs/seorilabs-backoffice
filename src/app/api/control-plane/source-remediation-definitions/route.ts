...
import { sourceRemediationDefinitionCreateSchema } from "@/lib/control-plane/contracts";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = sourceRemediationDefinitionCreateSchema.parse(await request.json());
    const result = await createSourceRemediationDefinition({
      ...body,
      actor: principal.id,
      idempotencyKey,
    });
