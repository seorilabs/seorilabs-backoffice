...
import { trustedLocalPendingSchema } from "@/lib/control-plane/contracts";
import { controlPlaneErrorResponse } from "@/lib/control-plane/http";
import {
  authenticateInternalRequest,
  requireIdempotencyKey,
} from "@/lib/control-plane/security";
import { markReauthTrustedLocalPending } from "@/lib/control-plane/service";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = trustedLocalPendingSchema.parse(await request.json());
