...
import { discoveryObservationSchema } from "@/lib/control-plane/contracts";
...
import { recordDiscoveryObservation } from "@/lib/control-plane/service";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = discoveryObservationSchema.parse(await request.json());
    const result = await recordDiscoveryObservation({
      ...body,
