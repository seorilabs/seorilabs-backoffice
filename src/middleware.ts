...
export const config = {
  ...
  // HMAC, api/internal/agents 와 agent-adapter, workflow-bundle-candidate-executor 는
  // worker/adapter bearer + principal + runtime binding으로 각 route가 fail-closed
  ...
  matcher: [
    "/((?!api/(?:webhooks|discord/interactions|auth|admin|control-plane|health|metrics)(?:/|$)|api/internal/platform/operational-events(?:/|$)|api/internal/(?:agents|agent-adapter|workflow-bundle-candidate-executor)(?:/|$)|_next/static|_next/image|favicon.ico|login(?:/|$)).*)",
  ],
};
