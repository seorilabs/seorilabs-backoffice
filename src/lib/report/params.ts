import { dbDay, lastElapsedMetricDay } from "@/lib/analytics/metric-day";
...
export function resolveReportRange(input: {
  ...
  const min = dbDay(new Date(Math.min(...mins.map((d) => d.getTime()))));
  const dataMax = dbDay(new Date(Math.max(...maxes.map((d) => d.getTime()))));
  // 미래 날짜가 실수로 적재돼도 피커 상한은 최신 확정일(D-1)을 넘지 않는다.
  const cap = lastElapsedMetricDay(input.now);
