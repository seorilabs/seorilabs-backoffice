...
/** 하루치 콘솔 지표(앱×날짜). 승격 스칼라 + 부가 원본(raw). AppConsoleMetricDaily 로 upsert 된다. */
export interface ConsoleDailyMetric {
  /** 기준일 "YYYY-MM-DD"(토스/KST). */
  date: string;
  /** 콘솔 활성 사용자(dashboard_dau.au). */
  dau: number;
  /** 신규 활성(dashboard_dau.newAu). */
  newUsers: number;
...
/** 한 앱의 push 묶음. slug 로 App 을 해석(miniAppId 는 검증/역방향 보조). */
export interface ConsoleAppPush {
  /** backoffice App.slug(= 콘솔 appName). 해석 1순위. */
  slug?: string;
  /** 콘솔 miniAppId. slug 미제공 시 App.aitMiniAppId 로 해석. */
  miniAppId?: number;
