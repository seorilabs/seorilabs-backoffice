...
import {
  ...
  type ContentSourceTarget,
} from "@/lib/analytics/content-source";

// ContentMetricsSource 의 GA4/BigQuery 구현. 스펙을 SQL 로 조립(순수 content-sql)해 events
// export 를 한 번 스캔하고, 응답을 마켓×날짜별 스냅샷으로 피벗한다. 향후 자체 지표 서버가
// 서면 같은 ContentMetricsSource 인터페이스의 HTTP 구현으로 교체한다.
...
export function mapRawContentRows(raw: Record<string, unknown>[]): ContentSqlRow[] {
  return raw.map((r) => ({
    date: String(r.date),
    market: String(r.market ?? MARKET_ALL),
    kind: String(r.kind),
