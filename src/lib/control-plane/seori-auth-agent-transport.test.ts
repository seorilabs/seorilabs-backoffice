...
test("K8s runtime과 client는 기본 scale 0, no service-account token, RPI5, 분리 secret, mTLS-only와 proxy-only egress로 선언된다", () => {
  ...
  assert.match(proxy, /SEORI_EGRESS_ALLOWED_HOSTS/u);
  assert.match(proxy, /SEORI_EGRESS_ALLOWED_CLIENT_SPIFFE_IDS/u);
  ...
  assert.match(canary, /"state":"CANARY_OK","secretExposed":false,"positive":1,"rejected":4/u);
  ...
  assert.match(canaryRunner, /"state":"CANARY_OK","secretExposed":false,"positive":1,"rejected":4/u);
