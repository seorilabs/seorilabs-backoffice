 ...
 * production에서는 k8s/operator-append-only-triggers-job.yaml만 사용한다.
 */
...
if (!new Set(["127.0.0.1", "localhost"]).has(databaseUrl.hostname)) {
  throw new Error("Trusted operator trigger fixture는 loopback MySQL에서만 허용한다");
}
if (!databaseUrl.pathname.slice(1).endsWith("_contract_test")) {
  throw new Error("Trusted operator trigger fixture DB 이름은 _contract_test로 끝나야 한다");
}
...
function executeTriggerDdl(statement: string): void {
  ...
  if (result.status !== 0) throw new Error("Trusted operator test trigger DDL 실패");
}
...
async function main(): Promise<void> {
  ...
  process.stdout.write(`Trusted operator test trigger 계약 통과: verified=${verified}\n`);
}

main()
  .catch((error: unknown) => {
    console.error(
      "Trusted operator test trigger 계약 실패:",
