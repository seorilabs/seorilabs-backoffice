...
7. 허용된 공개 결과만 complete/fail endpoint에 남기고 모든 결과에 이번 호출의 `costMicros`를 기록한다.
   ...
   `READBACK_FIRST`로 재claim해 받은 새 `sessionId`로만 readback resolution을 제출한다. trusted adapter가
   exact PR과 branch가 모두 없음을 서명한 경우에만 재개하며, Issue eligibility가 바뀌어도 새 작업을 만들지 않는다.
