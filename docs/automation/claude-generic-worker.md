...
1. `seorilabs-worker-contract.v1.json`의 `claim` endpoint를 `agentKind=CLAUDE`와 안정적인 worker ID로 호출한다.
...
7. 허용된 공개 결과만 complete/fail endpoint에 남긴다. 결과 불명은 공개 `RESULT_UNKNOWN` 결과로 readback-required를 기록한 뒤 동일 run의 readback resolution으로 닫는다.
   실행 중 Issue eligibility 변경으로 lease가 회수된 경우 새 작업을 만들지 말고 원래 token으로 GitHub 상태를 readback한 뒤 `readback`으로 닫는다.
