...
두 username은 각각 `fleet_migration_shadow`, `fleet_migration_proof_writer`로 고정한다.
...
- 공통 SELECT: `app`, `repository_registration`,
...
동시에 proof/claim/completion의 UPDATE와 DELETE를 막는 여섯 trigger를 0개일 때만
설치한다. 부분 설치, 변형, 추가 trigger는 자동 복구하지 않는다. 이후 고정 verifier가
기존 네 trigger와 합친 `total=10`, `exact=10`, repo contract digest를 새 migration
