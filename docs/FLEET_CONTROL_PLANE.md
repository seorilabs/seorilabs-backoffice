...
후보 static caller는 PR 경로만 실행한다. 번들 승격 뒤에는 APPROVED loader와 일반 static
generator로 main push·PR·dispatch caller를 다시 생성한다. 이 변경이 PR head를 바꾸면
동일한 advisory·만료 범위 안에서 exact head/merge/lock 후보 승인을 다시 고정해야 한다.
...
ACTIVE config의 source 결박은 관측 row id가 아니라 그 관측이 가리키는 source로 본다. 같은
...
`APP_PLATFORM_IDENTITY_MISSING`, `PLATFORM_FLEET_BINDING_MISSING` 관측으로 남긴다.
공개 증거는 연결 사실을 남기고 식별자만 `null`로 적으며, 릴리스가 없으면 기술할 대상이
없어 binding 자체를 `null`로 남긴다. readiness와 공개 증거가 같은 기준을 쓰므로 진단과
기록이 갈리지 않는다.
