...
flowchart LR
  ...
  C --> D["App 미러"]
  D --> E["앱 워크스페이스"]
  E --> F["표준 workflow dispatch"]
  F --> G["게임 운영 어댑터"]
  G --> H["workflow run webhook"]
  H --> D
```
...
- production 대상 지급·회수 workflow는 GitHub Environment 승인을 사용한다.
...
- 변경 완료는 workflow 성공뿐 아니라 실제 원격 설정 readback까지 검증한다.
