...
trusted operator는 `operator-append-only-triggers-job.yaml`로 legacy config resolution 두
trigger를 설치한다. 배포의 migration 사용자에게는 `SUPER`가 없고 binary logging이 켜져 있어
CREATE TRIGGER가 오류 1419로 막히기 때문이다. 이후 `status=PASS`, `total=6`, `exact=6`
readback을 확인하고 같은 main SHA 배포를 재실행한다. history row를 직접 고치거나
table·trigger를 drop하지 않으며 `SUPER`, `GRANT TRIGGER`,
`log_bin_trust_function_creators`도 변경하지 않는다.
복구 rollout 완료 뒤에는 새 backup을 만들고 격리 restore rehearsal로 schema·여섯 trigger·migration
lineage를 다시 검증한다.
...
- 지표와 임계(`METRIC_SPECS`, 2026-08-29 실측 분포 기준 — 전체 GA4 DAU 75명, 콘솔 일 광고수익 ₩38):

  | 지표 | 소스 | 최소 기준선 | 상대 임계 | 최소 절대 변화 | 최소 모수 |
  |---|---|---:|---:|---:|---:|
  | DAU | GA4 | 5명 | ±30% | 3명 | — |
  | D1 잔존율 | GA4 | 5% | ±15%p | — | 신규 20명 |
  | 보상형 광고 완료 | GA4 | 10회 | ±40% | 5회 | — |
  | 토스 DAU | 콘솔 | 5명 | ±40% | 3명 | — |
  | 광고 수익 | 콘솔 | ₩50 | ±50% | ₩50 | — |
  | 결제 거래액 | 콘솔 | ₩1,000 | ±40% | ₩1,000 | — |

  포트폴리오가 커지면 `minBaseline`·`minAbsDelta`를 함께 올린다.
- **상대 임계와 절대 임계를 동시에 요구한다.** 규모가 작으면 3명이 5명이 되어도 +67%라
  상대 임계만으로는 잡음이 리포트를 덮는다.
- **모수가 작은 지표는 판정하지 않는다.** D1은 신규 사용자 코호트가 20명 미만이면 값 자체가
  한두 명에 수십 %p 움직여 신호로 읽을 수 없다.
- **창 전체가 0인 지표는 "미집계"로 빼고 표본 부족에 세지 않는다.** 광고가 없는 앱의 광고
  수익까지 표본 부족으로 세면 리포트가 0으로 뒤덮인다.
...
- 콘솔은 온디맨드 push라 리스팅마다 최신일이 다르다. 기준일보다 오래된 스냅샷은 항목 줄에
  `⏳<날짜>`를 함께 찍는다.
- **유입경로**: 콘솔 `raw.referrer`를 리스팅 합산으로 접어 요약에 한 줄 붙인다. 비율은 리스팅별
  `rate` 평균이 아니라 유입 수 합으로 다시 계산한다 — 규모가 다른 리스팅의 비율을 평균하면 작은
  리스팅이 과대 대표된다. 수집 값이 없으면 줄 자체가 빠진다.
- **LLM 해설**: 판정·순위·수치는 전부 결정적 코드가 내고, LLM은 **이미 계산이 끝난 결과만** 받아
  한 문단(최대 300자)을 쓴다. 새 수치를 만들 재료를 주지 않으므로 환각이 리포트의 숫자를 오염시킬
  수 없다. 판정에서 빠진 항목은 앱 이름 없이 건수만 넘긴다 — 이름을 주면 없는 이야기를 만든다.
  `FEATURE_GEMINI_ENABLED` + `GEMINI_API_KEY`가 있어야 붙고, 미설정·실패·빈 응답이면 해설만
  빠지고 리포트는 그대로 나간다. 응답의 `narrated`로 붙었는지 확인한다. `ai_usage` path는
  `metric-narrative`.
